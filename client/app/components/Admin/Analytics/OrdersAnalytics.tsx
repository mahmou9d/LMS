...
const CustomTooltip = ({ active, payload, label, isDark }: TooltipProps) => {
  if (active && payload && payload.length) {
    return (
      ...
      >
        <p className="text-[11px] font-semibold text-purple-500 dark:text-purple-400 uppercase tracking-wider mb-1">
          {label}
        </p>
        <p
...
const OrdersAnalytics = ({ isDashboard }: Props) => {
  ...
  data?.orders.last12Months?.forEach((item: IMonthData) => {
  ...
  const tickColor = isDark ? "#cbd5e1" : "#94a3b8";
